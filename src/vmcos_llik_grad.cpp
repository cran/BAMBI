...
// [[Rcpp::export]]
double const_vmcos_mc(double k1, double k2, double k3, arma::mat uni_rand, int ncores = 1)
{
...
#pragma omp parallel for reduction(+:sum_exp) private(expon, x, y) num_threads(ncores)
  for(i = 1; i < nsim; i++) {
...
// [[Rcpp::export]]
arma::vec log_const_vmcos_all(arma::mat par_mat, arma::mat uni_rand, int ncores = 1) {
  int K = par_mat.n_cols;
  arma::vec all_lconsts(K);
  for(int j = 0; j < K; j++)
    all_lconsts[j] = log(const_vmcos(par_mat(0,j), par_mat(1,j), par_mat(2,j), uni_rand, ncores));
  return all_lconsts;
}
